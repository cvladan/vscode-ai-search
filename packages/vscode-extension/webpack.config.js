...
module.exports = {
    ...
    externals: {
        ...
        //
        // Native vector-database modules stay external so they are NOT bundled and are
        // require()'d at runtime from node_modules. They load lazily, only when the
        // LanceDB/Local backend is selected; the default Milvus (REST) backend needs
        // none of them, so the extension activates fine even if they are absent.
    ...
    plugins: [
        ...
        new CopyWebpackPlugin({
            patterns: [
                {
                    // Resolve from the installed package so it works whether node_modules
                    // is package-local or hoisted to the workspace root.
                    from: path.join(path.dirname(require.resolve('web-tree-sitter')), 'tree-sitter.wasm'),
