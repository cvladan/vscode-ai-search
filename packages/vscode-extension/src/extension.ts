 ...
 * the user's machine; Milvus (REST) is the default and needs no native modules.
 */
function createVectorDatabase(configManager: ConfigManager): any {
    const provider = configManager.getVectorDatabaseProvider();
    const dataDir = configManager.getVectorDatabaseDataDir();

    if (provider === 'LanceDB') {
        console.log(`Vector database initialized with LanceDB${dataDir ? ` (dir: ${dataDir})` : ''}`);
        return new LanceDBVectorDatabase({ dataDir });
    }
    if (provider === 'Local') {
        console.log(`Vector database initialized with Local FAISS+SQLite${dataDir ? ` (dir: ${dataDir})` : ''}`);
        return new LocalVectorDatabase({ dataDir });
    }
    ...
    if (milvusConfig) {
        ...
        return new MilvusRestfulVectorDatabase(milvusConfig);
    }
    console.log('No Milvus configuration found, using default REST API configuration');
    return new MilvusRestfulVectorDatabase({
...
function createContextWithConfig(configManager: ConfigManager): Context {
    ...
    try {
        ...
        // Create vector database instance (Milvus / LanceDB / Local)
        vectorDatabase = createVectorDatabase(configManager);
        contextConfig.vectorDatabase = vectorDatabase;

        // Optional reranker (second-stage relevance, e.g. OpenRouter Cohere rerank)
        const reranker = configManager.getReranker();
        if (reranker) {
            contextConfig.reranker = reranker;
            console.log(`Reranker initialized with ${reranker.getProvider()} (model: ${reranker.getModel()})`);
        }

        // Create splitter instance
        let codeSplitter;
        if (splitterConfig) {
            if (splitterConfig.type === SplitterType.RECURSIVE) {
                codeSplitter = new RecursiveCharacterSplitter(
...
function reloadContextConfiguration() {
    ...
    try {
        ...
        // Update vector database (Milvus / LanceDB / Local)
        codeContext.updateVectorDatabase(createVectorDatabase(configManager));

        // Update (or clear) the reranker
        codeContext.updateReranker(configManager.getReranker());

        // Update splitter if configuration exists
        if (splitterConfig) {
            let newSplitter;
            if (splitterConfig.type === SplitterType.RECURSIVE) {
                newSplitter = new RecursiveCharacterSplitter(
                    splitterConfig.chunkSize ?? 1000,
                    splitterConfig.chunkOverlap ?? 200
                );
            } else {
