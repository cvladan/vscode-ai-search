 ...
 * Provides true AST-based code splitting with fallback to recursive splitter
 */
...
class AstCodeSplitterStub {
    constructor(chunkSize = 2500, chunkOverlap = 300) {
        ...
        try {
            ...
            throw new Error('Both web-tree-sitter and the recursive splitter are unavailable');
        }
    ...
    async split(code, language, filePath) {
        // Markdown uses the dedicated mdast-based splitter when available
        const lang = (language || '').toLowerCase();
        if (this.markdownSplitter && (lang === 'markdown' || lang === 'md')) {
            try {
                console.log(`📝 [Markdown Splitter] Using mdast splitter for: ${filePath || 'unknown'}`);
                return await this.markdownSplitter.split(code, language, filePath);
            } catch (error) {
                console.warn('[Markdown Splitter] failed, falling back to recursive splitter:', error);
        ...
        try {
            ...
            if (!parserReady || !Parser) {
                console.log('[AST Splitter] web-tree-sitter not available, using recursive fallback');
                return this.fallbackSplitter.split(code, language, filePath);
            }

            const languageParser = await this.loadLanguage(language);
            if (!languageParser) {
                console.log(`[AST Splitter] Language ${language} not supported by web AST, using recursive fallback for: ${filePath || 'unknown'}`);
                return await this.fallbackSplitter.split(code, language, filePath);
            }

            // Ensure parser is available before setting language
            if (!this.parser) {
                console.warn(`[AST Splitter] Parser not initialized, falling back to recursive splitter: ${filePath || 'unknown'}`);
                return await this.fallbackSplitter.split(code, language, filePath);
            }
            ...
            if (!tree || !tree.rootNode) {
                console.warn(`[AST Splitter] Failed to parse AST for ${language}, falling back to recursive splitter: ${filePath || 'unknown'}`);
                return await this.fallbackSplitter.split(code, language, filePath);
            }
            ...
            console.warn(`[AST Splitter] web-tree-sitter failed for ${language}, falling back to recursive splitter:`, error);
            return await this.fallbackSplitter.split(code, language, filePath);
        }
    }
    ...
    async refineChunks(chunks, originalCode) {
        ...
        for (const chunk of chunks) {
            if (chunk.content.length <= this.chunkSize) {
                ...
                // Chunk is too large, split it using the recursive splitter
                console.log(`📏 [AST Splitter] Chunk too large (${chunk.content.length} chars), using recursive splitter for refinement`);
    ...
    setChunkSize(chunkSize) {
        ...
        if (this.markdownSplitter) this.markdownSplitter.setChunkSize(chunkSize);
    }

    setChunkOverlap(chunkOverlap) {
        this.chunkOverlap = chunkOverlap;
        this.fallbackSplitter.setChunkOverlap(chunkOverlap);
        if (this.markdownSplitter) this.markdownSplitter.setChunkOverlap(chunkOverlap);
    }
}
