...
export class SemanticSearchViewProvider implements vscode.WebviewViewProvider {
    ...
    resolveWebviewView(
        ...
        webviewView.webview.html = WebviewHelper.getHtmlContent(
            this._extensionUri,
            'dist/webview/templates/semanticSearch.html',
            webviewView.webview
        );
        ...
        webviewView.webview.onDidReceiveMessage(
            async message => {
                switch (message.command) {
                    ...
                    case 'rerank':
                        try {
                            const doRerank = message.command === 'rerank';
                            // Use search command (rerank only when explicitly requested)
                            ...
                            webviewView.webview.postMessage({
                                ...
                                query: message.text,
                                reranked: doRerank,
                                rerankAvailable: this.configManager.isRerankerConfigured()
                            });

                            vscode.window.showInformationMessage(
                                doRerank
                                    ? `Re-ranked ${results.length} results for: "${message.text}"`
                                    : `Found ${results.length} results for: "${message.text}"`
                            );
                        } catch (error) {
