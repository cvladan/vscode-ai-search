...
class SemanticSearchController {
    ...
    initializeElements() {
        ...
        this.rerankButton = document.getElementById('rerankButton');
    ...
    bindEvents() {
        ...
        this.rerankButton.addEventListener('click', () => this.performRerank());
    ...
    /**
     * Re-rank the current results on demand using the configured rerank model.
     */
    performRerank() {
        const text = (this.lastQuery || this.searchInput.value || '').trim();
        if (!text) {
            return;
        }
        const extFilterRaw = (this.extFilterInput?.value || '').trim();
        ...
        this.rerankButton.textContent = 'Re-ranking…';
        this.vscode.postMessage({
            command: 'rerank',
            text: text,
            fileExtensions: extensions
 ...
 */
     ...
     */
    showResults(results, query, reranked = false, rerankAvailable = false) {
        ...
        if (results.length === 0) {
            ...
            const suffix = reranked ? ' · re-ranked' : '';
            this.resultsHeader.textContent = `${results.length} result${results.length === 1 ? '' : 's'} for "${query}"${suffix}`;
            this.resultsList.innerHTML = results.map((result, index) => this.createResultHTML(result, index + 1)).join('');
        }

        // Offer the on-demand re-rank button only when a reranker is configured
        // and there is more than one result to reorder.
        this.rerankButton.disabled = false;
        this.rerankButton.textContent = reranked ? '↑ Re-rank again' : '↑ Re-rank results';
        this.rerankButton.style.display = (rerankAvailable && results.length > 1) ? 'block' : 'none';
    ...
    handleMessage(event) {
        ...
        switch (message.command) {
            case 'showResults':
                this.showResults(message.results, message.query, message.reranked, message.rerankAvailable);
                break;
