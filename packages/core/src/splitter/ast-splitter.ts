...
export class AstCodeSplitter implements Splitter {
    ...
    private fallback: any; // RecursiveCharacterSplitter for fallback
    private markdownSplitter: any; // MarkdownSplitter for markdown files
    ...
    constructor(chunkSize?: number, chunkOverlap?: number) {
        ...
        // Initialize markdown splitter (mdast-based)
        const { MarkdownSplitter } = require('./markdown-splitter');
        this.markdownSplitter = new MarkdownSplitter(chunkSize, chunkOverlap);
    }

    async split(code: string, language: string, filePath?: string): Promise<CodeChunk[]> {
        // Markdown gets its own structural (mdast) splitter
        const { MarkdownSplitter } = require('./markdown-splitter');
        if (MarkdownSplitter.isLanguageSupported(language)) {
            console.log(`📝 Using Markdown (mdast) splitter for: ${filePath || 'unknown'}`);
        ...
        if (!langConfig) {
            console.log(`📝 Language ${language} not supported by AST, using recursive splitter for: ${filePath || 'unknown'}`);
            return await this.fallback.split(code, language, filePath);
        }
        ...
        try {
            ...
            if (!tree.rootNode) {
                console.warn(`[ASTSplitter] ⚠️  Failed to parse AST for ${language}, falling back to recursive splitter: ${filePath || 'unknown'}`);
                return await this.fallback.split(code, language, filePath);
            }
            ...
            console.warn(`[ASTSplitter] ⚠️  AST splitter failed for ${language}, falling back to recursive splitter: ${error}`);
            return await this.fallback.split(code, language, filePath);
        }
    }
    ...
    setChunkSize(chunkSize: number): void {
        ...
        this.markdownSplitter.setChunkSize(chunkSize);
    }

    setChunkOverlap(chunkOverlap: number): void {
        this.chunkOverlap = chunkOverlap;
        this.fallback.setChunkOverlap(chunkOverlap);
        this.markdownSplitter.setChunkOverlap(chunkOverlap);
    }
