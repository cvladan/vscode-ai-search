...
let _sqliteOpen: any = null;
let _faiss: any = null;

function loadSqlite(): void {
    if (!_sqlite3) {
        _sqlite3 = require('sqlite3');
        _sqliteOpen = require('sqlite').open;
    }
}

function loadFaiss(): any {
    if (!_faiss) {
        _faiss = require('faiss-node');
    }
    return _faiss;
}
...
export class LocalVectorDatabase implements VectorDatabase {
    ...
    private async loadVectorIndex(collectionName: string, dimension: number): Promise<VectorIndex> {
        ...
        if (fs.existsSync(indexPath) && fs.existsSync(metadataPath)) {
            try {
                const index = loadFaiss().IndexFlatIP.read(indexPath);
        ...
        // Create new index
        const index = new (loadFaiss().IndexFlatIP)(dimension);
        const vectorIndex: VectorIndex = {
            dimension,
            index,
            idMapping: new Map(),
