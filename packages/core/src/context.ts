...
export class Context {
     ...
     */
    async semanticSearch(codebasePath: string, query: string, topK: number = 5, threshold: number = 0.5, filterExpr?: string, options?: { rerank?: boolean }): Promise<SemanticSearchResult[]> {
        const isHybrid = this.getIsHybrid();
        const searchType = isHybrid === true ? 'hybrid search' : 'semantic search';
        console.log(`[Context] 🔍 Executing ${searchType}: "${query}" in ${codebasePath}`);

        // Reranking is opt-in per request (triggered on demand from the UI), not automatic.
        const doRerank = !!(this.reranker && options?.rerank);

        // When reranking, retrieve a larger candidate pool from the vector DB and let
        // the rerank model pick the final topK from it.
        const candidateK = doRerank ? Math.max(topK, Math.min(50, topK * 5)) : topK;
        ...
        if (isHybrid === true) {
            ...
            return doRerank ? await this.maybeRerank(query, dedupedResults, topK) : dedupedResults.slice(0, topK);
        } else {
            ...
            const searchResults: VectorSearchResult[] = await this.vectorDatabase.search(
                ...
                { topK: candidateK, threshold, filterExpr }
            );
    ...
    /**
     * Update (or clear) the reranker instance.
     * @param reranker New reranker instance, or undefined to disable reranking.
     */
    updateReranker(reranker?: Reranker): void {
        this.reranker = reranker;
        console.log(`[Context] 🔄 Updated reranker: ${reranker ? `${reranker.getProvider()} (${reranker.getModel()})` : 'disabled'}`);
    }

    /**
     * If a reranker is configured, reorder the candidate results by model
     * relevance and trim to `topK`. On any failure, fall back to the original
     * (vector-search) ordering so search never breaks because of reranking.
     */
    private async maybeRerank(query: string, results: SemanticSearchResult[], topK: number): Promise<SemanticSearchResult[]> {
        if (!this.reranker || results.length <= 1) {
            return results.slice(0, topK);
        }
        try {
            const documents = results.map(r => r.content);
            const ranked = await this.reranker.rerank(query, documents, topK);
            ...
            console.log(`[Context] 🎯 Reranked ${results.length} candidates via ${this.reranker.getProvider()} (${this.reranker.getModel()}) -> top ${Math.min(reordered.length, topK)}`);
            return reordered.slice(0, topK);
        } catch (error) {
            console.warn('[Context] ⚠️  Rerank failed, keeping vector-search order:', error);
            return results.slice(0, topK);
        }
    }
        ...
        const traverseDirectory = async (currentPath: string) => {
            ...
            for (const entry of entries) {
                ...
                if (entry.isDirectory()) {
                    ...
                    // Skip files without an extension (e.g. Makefile, LICENSE, dotfiles)
 ...
 */
    ...
    private getLanguageFromExtension(ext: string): string {
        const languageMap: Record<string, string> = {
            ...
            '.sol': 'solidity',
            '.ipynb': 'jupyter',
            // Plain text is processed exactly like Markdown (mdast splitter)
            '.txt': 'markdown'
        };
    ...
    isLanguageSupported(language: string): boolean {
        ...
        // The recursive splitter supports any language
        return true;
    }
     ...
     * @param language Programming language
     */
    getSplitterStrategyForLanguage(language: string): { strategy: 'ast' | 'recursive'; reason: string } {
        const splitterName = this.codeSplitter.constructor.name;
        ...
        if (splitterName === 'AstCodeSplitter') {
            ...
            return {
                strategy: isSupported ? 'ast' : 'recursive',
                reason: isSupported
                    ? 'Language supported by AST parser'
                    : 'Language not supported by AST, will fall back to recursive splitter'
            };
        } else {
            return {
                strategy: 'recursive',
                reason: 'Using recursive splitter directly'
            };
        }
    }
}
